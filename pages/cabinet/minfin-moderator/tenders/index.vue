          ...
          <table>
            ...
            <tbody>
              <tr v-for="(item, index) in list" :key="item.id" @click="
                $router.push({
                  path: `/cabinet/minfin-moderator/tenders/detail/${item.id}`,
                })
              ">
...
export default {
  ...
  computed: {
    ...mapGetters('minfin/tender', ['list', 'count', 'loading', 'statusList']),
    params() {
      let params = {
  ...
  methods: {
    ...mapActions('minfin/tender', ['getList']),
    changeTab() {
