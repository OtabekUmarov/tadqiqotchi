<template>
  <layouts title="Budjet" :isHeader="false">
    <template #body>
      ...
      <div class="flex-center" v-if="detail.status == 'in_review_shaffof_moderator'">
        <a-button type="primary" class="grey-reject mr-20" @click="openReject">Rad etish</a-button>
        <a-popconfirm title="Tenderni tasdiqlamoqchimisiz?" ok-text="Ha" cancel-text="Yo'q"
          @confirm="setStatus({ form })">
          <a-button type="primary" class="green"> Tasdiqlash </a-button>
        </a-popconfirm>
      </div>
      <PopupsModeratorTenderReject :setStatus="setStatus" :isVisible="isReject" @close="closeReject" />
    </template>
  </layouts>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters('moderator/tender', ['detail', 'loading'])
  },
  methods: {
    ...mapActions('moderator/tender', ['getDetail', 'putStatus']),
    openReject() {
      this.isReject = true
    },
    closeReject() {
    ...
    async setStatus({ form }) {
      await this.putStatus({ form, id: this.$route.params.id });
      this.form.reject_comment = null;
