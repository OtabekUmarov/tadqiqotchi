      ...
      <template v-else>
        <a-row :gutter="[20, 24]" v-if="layoutId == 1" type="flex">
          <a-col span="12" v-for="item in list" :key="item.id">
            <CardsModeratorTender :data="item" />
          </a-col>
        </a-row>
        <div class="table" v-else>
          <table>
            ...
            <tbody>
              <tr v-for="(item, index) in list" :key="item.id" @click="
                $router.push({
                  path: '/cabinet/moderator/tenders/detail/' + item.id,
                })
              ">
...
export default {
  ...
  computed: {
    ...mapGetters('moderator/tender', ['list', 'count', 'loading', 'statusList']),
    params() {
      let params = {
  ...
  methods: {
    ...mapActions('moderator/tender', ['getList']),
    changeTab() {
