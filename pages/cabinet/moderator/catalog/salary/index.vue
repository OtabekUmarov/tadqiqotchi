<template>
  <layouts title="Eng kam ish haqi">
    ...
    <template #body>
        ...
        <div class="table">
          <table>
            <thead>
              <tr>
                <th width="1%">№</th>
                <th>Eng kam ish haqi miqdori</th>
                <th width="1%"></th>
              </tr>
            </thead>
            <tbody>
      ...
      <popupsLayout @close="close" :isFooter="true" title="Qo'shish" :isVisible="isVisible">
        <template #body>
          <a-form-item label="Eng kam ish haqi miqdori">
...
export default {
  ...
  computed: {
    ...mapGetters("moderator/catalog/salary", ["list", "count", "loading"]),
    params() {
      let params = {
  ...
  methods: {
    ...mapActions("moderator/catalog/salary", ["getList", "postDetail", "deleteDetail"]),
    async send() {
      this.pending = true;
      try {
