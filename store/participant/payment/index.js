import statusList from "@/constants/participantPaymentStatus";
export const state = () => ({
    list: [],
    detail: {},
    loading: false,
...
export const actions = {
    getList({ commit }, { params }) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const { data } = await this.$axios.get("/admin/applicant/participant/payment/list/", { params })
    ...
    getDetail({ commit }, { id }) {
        return new Promise(async (resolve, reject) => {
            try {
                commit("setState", { key: "loading", payload: true })
                const { data } = await this.$axios.get(`/admin/applicant/participant/payment/${id}/detail/`)
