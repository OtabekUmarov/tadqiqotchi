    ...
    getList({ commit }, { params }) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const { data } = await this.$axios.get("/admin/rating/list/", { params })
    ...
    async postDetail(_, { form }) {
        try {
            ...
            if (form.id) {
                let id = form.id
                delete form.id
                res = await this.$axios.put(`/admin/rating/${id}/update/`, form)
            } else {
              res = await this.$axios.post('/admin/rating/create/', form) }
